...
En Java, ça se traduit par : **une référence de type parent peut pointer vers un objet de type enfant**, et c'est la méthode de l'enfant qui sera exécutée (si elle est redéfinie).
...
1. Le **polymorphisme dynamique** (*runtime*, à l'exécution) — via l'héritage ou les interfaces à l'aide de la redéfinition (*overriding*). C'est comme dans l'exemple précédent, résolu à l'exécution par la JVM.
2. Le **polymorphisme statique** (*compile-time*, à la compilation) — via la surcharge de méthodes (*overloading*), où plusieurs méthodes portent le même nom mais des signatures différentes. C'est comme dans l'exemple suivant :
