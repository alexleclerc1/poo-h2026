...
À partir du diagramme UML et des contraintes suivantes, faire l'implémentation complète de la classe `Inventaire`, servant à manipuler un conteneur de type `ArrayList<Article>` :

### Diagramme UML

![Diagramme UML de la classe Inventaire](../images/uml_Inventaire.png)

### Contraintes

- `supprimerParCode` supprime le premier article trouvé et renvoie `true` ou `false` selon que la suppression a eu lieu ou non;
- `rechercherParCode` renvoie `null` si aucun élément n'est trouvé, sinon le premier objet trouvé est renvoyé;
- `listerActifs` renvoie une nouvelle liste (une copie, pas la référence interne!);
- `valeurTotaleAvecTaxes` = somme des prix avec taxes de tous les articles **actifs** avec précision à `0.01` (arrondir à 2 décimales).

### Jeux de tests

Dans la méthode `main` de la classe `Inventaire`, instancier un objet de la classe `Inventaire`, puis :

- a) Tester la méthode `ajouter` avec les objets suivants :

    ```java
    new Article("CLV-USB-01", "Clavier USB", 29.99, true);
    new Article("SRC-WLS-77", "Souris sans fil", 19.95, true);
    new Article("SCR-24-FHD", "Écran 24 pouces FHD", 249.99, true);
    new Article("AUD-HS-12", "Casque audio", 89.50, false);
    new Article("CAM-HD-09", "Webcam HD", 59.99, true);
    new Article("SSD-1T-EXT", "Disque SSD externe 1 To", 129.99, false);
    new Article("LAP-15-I7", "Portable 15 pouces i7", 999.00, true);
    new Article("PRN-LSR-02", "Imprimante laser", 199.95, true);
    new Article("RTR-WIFI-AX", "Routeur Wi-Fi AX", 149.99, false);
    new Article("USB-64-BLK", "Clé USB 64 Go", 14.99, true);
    ```

    Puis tester `toString` pour afficher la liste après les ajouts.

- b) Tester la méthode `supprimerParCode` en saisissant un code de la part de l'utilisateur en console. Afficher si la suppression a eu lieu ou non.
- c) Tester la méthode `valeurTotaleAvecTaxes` en affichant le total en console.
- d) Tester la méthode `desactiver` de la classe `Article` en désactivant le premier article qui est actif dans l'`ArrayList`.
- e) Tester la méthode `activer` de la classe `Article` en activant TOUS les articles dans l'`ArrayList`. Refaire un appel à `valeurTotaleAvecTaxes` en affichant le nouveau total en console, puis refaire afficher la liste avec `toString` une dernière fois.
